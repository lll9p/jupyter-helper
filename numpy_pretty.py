...
# from https://notebooks.azure.com/eric-wieser
...
def _html_repr_helper(contents, index, is_horz):
    ...
    if dims_left == 0:
        ...
        s = '<span class="numpy-array-comma">,</span>'.join(
            _html_repr_helper(contents, index + (i,), is_horz)
            for i in range(contents.shape[len(index)])
        )
        s = (
            '<span class="numpy-array-bracket numpy-array-bracket-open">[</span>'
            "{}"
            '<span class="numpy-array-bracket numpy-array-bracket-close">]</span>'.format(
                s
            )
        )
...
def make_pretty(
    self, show_brackets=False, is_horz=lambda arr, ax: ax == arr.ndim - 1
):
    ...
    return IPython.core.display.HTML(
        """<style>{}</style><div class='{}'>{}</div>""".format(
            css, " ".join(classes), _html_repr_helper(self, (), is_horz)
        )
    )