...
def _import_libs(module_groups: list,
                 module_dict: dict,
                 excludes: dict,
                 scope: dict) -> None:
    for module_group in module_groups:
        ...
        for (_name, _sub), imp_name in module_dict.get(module_group).items():
            ...
            scope[imp_name] = _lib
            print(f'{_name}{"."+_sub if _sub else ""} → {imp_name}', end=', ')
        print()
    return None
...
def get_args() -> None:
    ...
    parser.add_argument(
        "--reset",
        type=str,
        default="reset -f in out dhist array")
    ...
    return autoreload_num, figure_format, module_arg, \
        random_state, sns_style, styles_dict
...
def run_magics(autoreload_num, figure_format) -> None:
    ...
    if 'autoreload' not in get_ipython().magics_manager.magics['line']:
        # if autoreload ext was loaded, loaded it.
        get_ipython().magic('load_ext autoreload')
    get_ipython().magic(f'autoreload {autoreload_num}')
    print(f'Ext. autoreload is loaded and set to {autoreload_num} .')


def main(_globals: dict) -> None:
    current_path = os.path.split(__file__)[0]
    parent_path = os.path.split(current_path)[0]
    autoreload_num, figure_format, module_arg, \
        random_state, sns_style, styles_dict = get_args()
    ...
    if short_names['seaborn'] in _globals:
        sns.set(context=styles_dict.get('context', 'notebook'),
...
if __name__ == "__main__":
    ...
    --matplotlib=inline \
    --modules=accelerate,base-numpy,plot-plotly-matplotlib.pyplot,symbol \
    --random_state=42 \
    --sns_style=context:notebook,style:ticks,font_scale:1.2,\
